...
CRITICAL — DOUBLE-COUNTING GUARD: Each nonzero delta must be justified by a DISTINCT evidence atom. A single headline justifies ONE nonzero node delta only. If the same event plausibly fires multiple nodes, score ONLY the most direct mechanism and zero the rest. Examples: "Deal signed" → PreferenceAlignment only. "Ceasefire violated, retaliatory strike" → HardlineClaims only if outside ceasefire framework, else 0. "Arms sale approved" → PatronDeterrence only. "Mobilization order" → MobilizationSignal only. "New sanctions plus threatening rhetoric, no territorial claim named" → PreferenceAlignment only, NOT HardlineClaims (audit finding 2026-07-27: this exact pattern was firing both nodes off one underlying story). When in doubt, score the most structurally upstream node and zero the rest.

- WinProbability: confirmed destruction/degradation of military assets, major military casualties, force reinforcement (e.g. a carrier strike group deployed to the region, additional troops/equipment committed), or withdrawal. SCOPE GUARD (Phase 2, 2026-09-10): a strike or attack occurring, by itself, is NOT sufficient -- that is LiveViolenceObserved's job (and, for the initiator's broader retaliation posture, the theater-hazard mechanism's). Only score here when the story confirms the strike's OUTCOME materially changed relative capability ("destroyed several fighter jets on the ground", "degraded the air defense network", "dozens of soldiers killed when the base was overrun") or when there's a distinct capability-shift story with no violence at all ("US deploys a second carrier strike group to the region"). Concrete example of what does NOT qualify: "Iran launched a missile strike on a US base in Jordan" with no confirmed damage or casualties reported -- real escalation, but LiveViolenceObserved only; WinProbability stays 0 until/unless a follow-up story confirms material damage. Do NOT score routine exercises, patrols, or chronic presence here -- those belong to RoutineMilitaryPressure, unless the story is specifically about a force-posture CHANGE (surge, reinforcement, withdrawal), not routine presence. Do NOT score mobilization/call-up/conscription orders here -- those belong to MobilizationSignal.
...
- HardlineClaims: territorial rhetoric, sovereignty claims escalating → +0.5. De-escalation → -0.5. IMPORTANT: retaliatory strikes or exchanges that occur WITHIN an active ceasefire/truce framework score 0 — the ceasefire framework is the dominant mechanism (PreferenceAlignment), not the individual exchange. Only score +0.5 for NEW escalation outside any existing framework. SCOPE GUARD (added 2026-07-27, audit finding): this node requires an ACTUAL territorial or sovereignty claim/dispute -- a named piece of land, maritime zone, or explicit sovereignty assertion. General hostile rhetoric, threats of future unspecified action, new sanctions, or a leader signaling a country "could be next" are real escalation but have ZERO territorial content -- they belong to PreferenceAlignment (diplomatic-relationship trajectory) or AudienceCosts, not here. Concrete example of what does NOT qualify: "Trump says Cuba could be next after Iran" plus new sanctions on Cuban officials -- genuine escalation, but no territorial claim anywhere in it, so PreferenceAlignment only, HardlineClaims stays 0. Before scoring nonzero here, be able to name the specific territory or sovereignty claim in question -- if you can't, the answer is 0.
